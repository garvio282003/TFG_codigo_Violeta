...
def load_model(model_name, _dev=None):
    if model_name == 'mt5':
        _mdl = MT5ForConditionalGeneration.from_pretrained(
            "google/mt5-base",
            device_map="auto",
        ...
        sys.exit(1)
    return _mdl.to(_dev) if _dev else _mdl

def load_tokenizer(args):
	if args.model_name == 'mbart':
		_tok = MBart50Tokenizer.from_pretrained("facebook/mbart-large-50")
		...
		_tok = AutoTokenizer.from_pretrained("google/mt5-base")
...
def read_parameters():
	...
	parser.add_argument("-dir", "--folder", default="./dataset_nllb", help="Folder where is the dataset")
	parser.add_argument('-model','--model_name',default='mt5',choices=['mbart','m2m','flant5','mt5','llama3','nllb'],help='Model to train')
...
def main():
	...
	training_args = Seq2SeqTrainingArguments(
		'{0}_{1}'.format(args.model_name,args.source+args.target),
		save_strategy="steps",
		eval_strategy='steps',
		eval_steps=5000,
		learning_rate=5e-4,
	...
	metrics_file = f"{args.model_name}_{args.source}_{args.target}_metrics.txt"
	try:
		results = trainer.evaluate()
		with open(metrics_file, 'w') as f:
			f.write(f'Antes de fine-tuning:\n')
			f.write(f'\tBLEU = {results["eval_bleu"]:.4f}\n')
			if "eval_ter" in results:
				f.write(f'\tTER = {results["eval_ter"]:.4f}\n')
	except Exception as e:
		print(f"Error en evaluación inicial: {e}")
		with open(metrics_file, 'w') as f:
			f.write(f'Error en evaluación inicial: {e}\n')
	...
	try:
		...
		print(f"ENTRENAMIENTO INTERRUMPIDO")
		print(f"Error: {e}")
		...
		with open(metrics_file, 'a') as f:
			...
			if hasattr(trainer, 'state') and trainer.state is not None:
				f.write(f'Último epoch completado: {trainer.state.epoch:.2f}\n')
				f.write(f'Último step: {trainer.state.global_step}\n')
	
	try:
		results = trainer.evaluate(eval_dataset=dataset["test"])
		with open(metrics_file, 'a') as f:
			f.write(f'\nDespues de fine-tuning:\n')
			f.write(f'\tBLEU = {results["eval_bleu"]:.4f}\n')
			if "eval_ter" in results:
				f.write(f'\tTER = {results["eval_ter"]:.4f}\n')
			if hasattr(trainer, 'state') and trainer.state is not None:
				f.write(f'\tEpochs completados: {trainer.state.epoch:.2f}/{args.epochs}\n')
				f.write(f'\tSteps completados: {trainer.state.global_step}\n')
	except Exception as e:
		print(f"Error en evaluación final: {e}")
		with open(metrics_file, 'a') as f:
			f.write(f'\nError en evaluación final: {e}\n')
