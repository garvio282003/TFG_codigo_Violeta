...
def load_model(model_name, args, _dev=None):
	if model_name == 'marian':
		_mdl = AutoModelForSeq2SeqLM.from_pretrained(f"Helsinki-NLP/opus-mt-{args.source}-{args.target}",
...
def load_tokenizer(args):
	...
	elif args.model_name == 'marian':
		_tok = AutoTokenizer.from_pretrained(f"Helsinki-NLP/opus-mt-{args.source}-{args.target}")
	else:
...
def read_parameters():
	...
	parser.add_argument('-model','--model_name',default='marian',choices=['marian','mbart','m2m','flant5','mt5','llama3','nllb'],help='Model to train')
...
def main():
	...
	with open(metrics_file, 'w') as f:
		f.write(f'Antes de fine-tunning:\n')
